...
/*
 * Temporary in-memory Blog data store for Assessment 2.
 * Data resets whenever the Node.js server restarts.
 *
 * Public pages only expose posts whose status is "published".
 * Drafts are kept for their owner and will be used later by my_posts.ejs,
 * create_post.ejs, and post_edit.ejs.
 */
...
const posts = [
    {
        ...
        tags: ["Mission", "Development", "Vietnamese Craft", "Social Impact"],
        status: "published",
        isLead: true,
        isFeatured: false,
        content: [
    ...
    },
    {
        ...
        tags: ["Guide", "Provenance", "Research"],
        status: "published",
        isLead: false,
        isFeatured: false,
        content: [
    ...
    },
    {
        ...
        tags: ["Donation", "Community", "Support"],
        status: "published",
        isLead: false,
        isFeatured: false,
        content: [
...
const findPostIndex = (postId) => {
    const id = normaliseId(postId);
    return posts.findIndex((post) => post.id === id);
};

const getMutablePost = (postId) => {
    const index = findPostIndex(postId);
    return index === -1 ? null : posts[index];
};

const sortNewestFirst = (postA, postB) => {
    const dateA = new Date(postA.publishedAt || postA.updatedAt || 0).getTime();
    const dateB = new Date(postB.publishedAt || postB.updatedAt || 0).getTime();
    return dateB - dateA;
};

const createSlug = (title) => {
    const baseSlug = String(title || "post")
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
...
const getPublishedPosts = () =>
    clone(posts.filter((post) => post.status === "published").sort(sortNewestFirst));

const getPostById = (postId) => {
    const post = getMutablePost(postId);
    return post ? clone(post) : null;
};

const getVisiblePostById = (postId, viewerId = null) => {
    const post = getMutablePost(postId);
    ...
    const canViewDraft =
        ...
        normaliseId(viewerId) === post.author.id;
...
const getLeadStory = () => {
    const post = posts.find(
        (item) => item.status === "published" && item.isLead,
    );
    return post ? clone(post) : null;
};
...
const getRelatedPosts = (postId, limit = 3) => {
    const sourcePost = getMutablePost(postId);
    ...
    const otherCategories = posts.filter(
        (post) =>
            post.status === "published" &&
            post.id !== sourcePost.id &&
            post.category !== sourcePost.category,
...
const getPostsByAuthorId = (authorId) =>
    clone(
        posts
            .filter((post) => post.author.id === normaliseId(authorId))
            .sort(sortNewestFirst),
    );

const countPostsByAuthorId = (authorId) =>
    posts.filter((post) => post.author.id === normaliseId(authorId)).length;
...
const createPost = (postData, owner) => {
    if (!owner || !normaliseId(owner.id)) {
        throw new Error("A logged-in owner is required to create a post.");
    }

    const now = new Date().toISOString();
    const status = allowedStatuses.has(postData.status)
        ? postData.status
        : "draft";

    const post = {
        id: createSlug(postData.title),
        title: String(postData.title || "Untitled draft").trim(),
        category: String(postData.category || "Guide").trim(),
        author: {
            id: normaliseId(owner.id),
            name: String(owner.name || "Current user").trim(),
            initials: String(owner.initials || getInitials(owner.name)).trim(),
            role: String(owner.role || "Author").trim(),
        },
        publishedAt: status === "published" ? now : null,
        updatedAt: now,
        readTime: Number(postData.readTime) || 1,
        summary: String(postData.summary || "").trim(),
        archiveSummary: String(
            postData.archiveSummary || postData.summary || "",
        ).trim(),
        image: {
            url: String(postData.imageUrl || "").trim(),
            listUrl: String(postData.imageUrl || "").trim(),
            alt: String(postData.imageAlt || postData.title || "Blog image").trim(),
            caption: String(postData.imageCaption || "").trim(),
            listCaption: String(postData.imageCaption || "").trim(),
        },
        tags: Array.isArray(postData.tags) ? [...postData.tags] : [],
        status,
        isLead: Boolean(postData.isLead),
        isFeatured: Boolean(postData.isFeatured),
        content: Array.isArray(postData.content) ? clone(postData.content) : [],
        comments: [],
    };
...
const updatePost = (postId, ownerId, updates = {}) => {
    const post = getMutablePost(postId);
    ...
    if (post.author.id !== normaliseId(ownerId)) {
    ...
    editableFields.forEach((field) => {
        if (Object.prototype.hasOwnProperty.call(updates, field)) {
            post[field] = clone(updates[field]);
        }
    });
    ...
    if (allowedStatuses.has(updates.status)) {
        ...
        if (wasDraft && updates.status === "published" && !post.publishedAt) {
...
const saveDraft = (postId, ownerId, updates = {}) =>
    updatePost(postId, ownerId, {
        ...updates,
        status: "draft",
    });

const publishPost = (postId, ownerId, updates = {}) =>
    updatePost(postId, ownerId, {
        ...updates,
        status: "published",
    });

const deletePost = (postId, ownerId) => {
    const index = findPostIndex(postId);
    ...
    if (posts[index].author.id !== normaliseId(ownerId)) {
        return { ok: false, reason: "forbidden" };
    }

    const [deletedPost] = posts.splice(index, 1);
    return { ok: true, post: clone(deletedPost) };
};
...
const addComment = (postId, commentData) => {
    ...
    const authorName = String(commentData.authorName || "Guest").trim();
    const authorId = normaliseId(commentData.authorId);
    ...
    const comment = {
        ...
        author: {
            ...
            name: authorName,
            initials: String(
                commentData.authorInitials || getInitials(authorName),
            ).trim(),
            email: String(commentData.authorEmail || "").trim(),
        },
...
const deleteComment = (postId, commentId, actorId) => {
    const post = getMutablePost(postId);
    ...
    const commentIndex = post.comments.findIndex(
        (comment) => comment.id === normaliseId(commentId),
    );
    ...
    const canDelete =
        actor &&
        (comment.author.id === actor || post.author.id === actor);
    ...
    return { ok: true, comment: clone(deletedComment) };
};
