(() => {
  ...
  const updateImageStatus = () => {
     ...
     * disabled file inputs are not
     * included in multipart requests.
     */
    if (imageInput) {
  ...
  const saveDraft = () => {
    ...
    if (draftStatus) {
      draftStatus.textContent =
        "Text draft saved in this browser. Images are not stored.";
    }
  };
  ...
  const restoreDraft = () => {
    ...
    try {
      ...
      if (draftStatus) {
        draftStatus.textContent =
          "Text draft restored. Select photos before publishing.";
      }
    } catch {
      clearStoredDraft();
    }
  ...
  /*
   * Explicitly append the current selectedFiles
   * collection to the native multipart request.
   */
  form.addEventListener(
    "formdata",
    (event) => {
  ...
  const getCardSearchValue = (
    ...
    if (
      ...
      return card.dataset.description || "";
    }
  ...
  const compareCards = (
    ...
    const firstRating =
      Number(
        first.dataset.rating
      );

    const secondRating =
      Number(
        second.dataset.rating
      );
      ...
      (button) => {
        button.addEventListener(
          ...
          (event) => {
            const confirmed =
              window.confirm(
                "Delete your review and its uploaded photos? This action cannot be undone."
              );
