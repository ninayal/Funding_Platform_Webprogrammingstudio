...
const postForgotPassword = async (
  ...
  if (user) {
    await passwordResetModel.createRequest(
      user.email
    );
  }
...
const getAdminPage = async (
  ...
  const pendingRequests =
    await passwordResetModel.getPendingRequests();

  const resolvedRequests =
    await passwordResetModel.getResolvedRequests();
...
const getProductFormPage = async (
  req,
  res
) => {
  const product =
  ...
  return res.render(
    "shared/admin/product_form",
    {
      ...
      craftVillages:
        adminProductModel.CRAFT_VILLAGES,

      materials:
        adminProductModel.MATERIALS,
...
const postUpdateProduct = async (
  ...
  await adminProductModel.updateProduct(
    id,
    {
      title: fields.title,
      category: fields.category,
