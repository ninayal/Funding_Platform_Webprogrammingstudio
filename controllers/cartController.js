...
const getCartPage = async (
  ...
  try {
    ...
    const recommendedProducts =
      await productModel.getRecommendedProducts(
        excludedIds,
        4,
        statsMap
      );
...
const submitCheckout = async (
  ...
  try {
    ...
    const pendingGiftcards =
      await cartModel.getPendingGiftcardDrafts(
        userId
      );

    const createdEntries =
      await Promise.all(
        pendingGiftcards.map(
          async (item) => [
            item.productId,
            await giftcardModel.createGiftcard(
              item.values,
              userId
            ),
          ]
