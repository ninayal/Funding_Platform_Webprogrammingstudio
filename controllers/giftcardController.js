...
const getCurrentUser=(req)=>req.currentUser||req.session?.user||null;
...
const renderGiftcard=(req,res,values,extras={},status=200)=>
  res.status(status).render("giftcard/giftcard",buildGiftcardPageData(values,{
...
const getGiftcardPage=(req,res,next)=>{
  try{
    ...
    }else if(req.query.review!=="1"&&req.session?.giftcardCartItemId){
      delete req.session.giftcardCartItemId;
    }

    const draft=req.session?.giftcardDraft||null;
    const reviewMode=Boolean(currentUser&&draft&&req.query.review==="1");
...
const reviewGiftcard=(req,res,next)=>{
  try{
    ...
    if(!valid)return renderGiftcard(req,res,values,{errors},422);
